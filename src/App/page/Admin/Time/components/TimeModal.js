...
function BasketModal({
  ...
  const uploadAudioReq = async (audio, index) => {
    ...
    return { ...time.group[index], audio: audioReq.data };
  };
  const onSubmit = async () => {
    let dataUpdate = { ...time, lang: time.lang._id };
    if (picture.formFile !== "") {
      const resImage = await uploadImageReq();
      if (resImage !== "")
        dataUpdate = { ...dataUpdate, image: { ...resImage.image } };
    }
    const resAudio = time.group.map((item, index) => {
    ...
    await updateLanguageAction(dataUpdate);
